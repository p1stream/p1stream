...
exports.headers = function(videoHeader, audioHeader, ctx) {
    var b = new Buffer(376);
    var i = 0;
    ...
    b[i+2] = 0x00;  // PAT PID
    b[i+3] = 0x10;  // Have payload
    i = padPacket(b, i, 167);

    // Table header
    b[i+0] = 0x00;
    b[i+1] = 0x00;  // PAT table ID
    b[i+2] = 0xB0;
    b[i+3] = 13;  // Length

    // Table syntax header
    b[i+4] = 0x00;
    b[i+5] = 0x00;  // Stream ID
    b[i+6] = 0xC1;
    b[i+7] = 0x00;
    b[i+8] = 0x00;
    ...
    // Single program
    b[i+ 9] = 0x00;
    b[i+10] = 0x21;  // Program number
    b[i+11] = 0xE0;
    b[i+12] = 0x21;  // PMT PID

    // CRC
    b.writeInt32BE(crc32.buf(b.slice(i, i + 13)), i + 13);
    ...
    // Table header
    b[i+0] = 0x00;
    b[i+1] = 0x02;  // PMT table ID
    b[i+2] = 0xB0;
    b[i+3] = 23;  // Length

    // Table syntax header
    b[i+4] = 0x00;
    b[i+5] = 0x21;  // Program number
    b[i+6] = 0xC1;
    b[i+7] = 0x00;
    b[i+8] = 0x00;
    ...
    // CRC
    b.writeInt32BE(crc32.buf(b.slice(i, i + 23)), i + 23);

    // Start with TS header and video header
    return Buffer.concat([b, exports.videoFrame(videoHeader, ctx)]);
};

exports.videoFrame = function(frame, ctx) {
    // Rewrite to Annex B
    var b = new Buffer(frame.buf);
    frame.nals.forEach(function(nal) {
        b.writeUInt32BE(1, nal.start - frame.start);
    });

    return packFrame(b, ctx, 0x23, 0xE0, frame.pts, 0);
};

exports.audioFrame = function(frame, ctx) {
    // ADTS header
    ...
    // Transmit PCR
    var pcr = 0;
    var pcrDiff = frame.pts - ctx.lastPcr;
    if (!ctx.lastPcr || frame.pts - ctx.lastPcr > 25000000)
        pcr = ctx.lastPcr = frame.pts;

    return packFrame(b, ctx, 0x22, 0xC0, frame.pts, pcr);
};

function packFrame(inb, ctx, pid, streamId, pts, pcr) {
    ...
    while (bytes) {
        ...
        var dlen = (bytes > available ? available : bytes);
        ...
        b[i+2] = pid;  // PID
        b[i+3] = 0x10 | seq;  // Have payload, seq
        if (!inpos && pcr) {
