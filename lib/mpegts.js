...
module.exports = function(mixer, dataCb) {
    ...
    function onVideoFrame(frame) {
        if (frame.keyframe) {
            dataCb(buildHeaders());
            dataCb(buildVideoFrame(mixer.$videoHeaders));
        }
    ...
    function buildAudioFrame(frame) {
        ...
        // Transmit PCR
        var pcr = 0;
        var pcrDiff = frame.dts - lastPcr;
        if (!lastPcr || frame.dts - lastPcr > 25000000)
            pcr = lastPcr = frame.dts;

        return packFrame(b, 0x22, 0xC0, frame.dts, pcr);
    }
